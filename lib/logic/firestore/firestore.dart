...
class Firestore {
  ...
  static Future<ChatMessage> getLastChatMessages(DocumentReference chatRef,
    ...
    if (currentMessage != null && lastDate == null) {
      ...
      currentMessage!.sort((element1, element2) {
        var bebra = element1.id.split("|").last;
        var bebra2 = element2.id.split("|").last;
        return DateTime.parse(bebra).compareTo(DateTime.parse(bebra2));
      });
      for (int i = currentMessage.length - 1; i > 0; i--) {
        ...
        if (DateTime.parse(currentMessage![i].id.split("|").last)
            .isBefore(lastDate)) {
          currentMessage = [currentMessage[i + 1]];
          break;
        }
      }
    }
  ...
  static Future<Message?> getLastActualMessage(ChatModel chat) async {
    ...
    for (var chatMessage in chat.chatMessages!.reversed.toList()) {
